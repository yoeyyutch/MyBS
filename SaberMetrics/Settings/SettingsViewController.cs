...
namespace SaberMetrics.Settings
{
	public class SettingsViewController : PersistentSingleton<SettingsViewController>
	{
		...
		public void OnApply()
		{
			//Plugin.Log?.Info($"Apply: enabled={_modEnabled}, strength={_strength}, duration={_duration}, strength_saber={_strength_saber}, strength_wall={_strength_wall}, strength_ui={strength_ui}");
			PluginConfig.Instance.Enabled = _modEnabled;
		...
		private void Awake()
		{
			...
			//Plugin.Log?.Info($"XRDevice.model: {XRDevice.model}");

			//_isOculusPlatform = XRSettings.loadedDeviceName.IndexOf("oculus", StringComparison.OrdinalIgnoreCase) >= 0;
			//Plugin.Log?.Info($"XRSettings.loadedDeviceName: {XRSettings.loadedDeviceName}");

			//ReadCurrentSettings();
			//Plugin.Log?.Info($"Awake: enabled={_modEnabled}, strength={_strength}, duration={_duration}, strength_saber={_strength_saber}, strength_wall={_strength_wall}, strength_ui={_strength_ui}");
		}
		...
		{
			//Plugin.Log?.Info($"RumbleTest: strength={strength}, duration={duration}");
			...
			//else
			//{
			//    Plugin.Log?.Critical("Error getting HapticFeedbackController");
			//}
		}
	}
}
